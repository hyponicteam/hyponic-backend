...
trait GetGrowthData
{
    private function getGrowthPerDays($data, $column_name)
    {
        ...
        for ($i = 0; $i < count($data) - 1; $i++) {
            ...
            $duration = round(Carbon::parse($data[$i]->created_at)->diffInSeconds(Carbon::parse($data[$i + 1]->created_at), true) / 86400, 2);
            
            $plant_height_growth = round($growth / $duration, 2);

            array_push($growth_per_days, [
                'from' => Carbon::parse($data[$i + 1]->created_at)->toDateTimeString(),
                'to' => Carbon::parse($data[$i]->created_at)->toDateTimeString(),
